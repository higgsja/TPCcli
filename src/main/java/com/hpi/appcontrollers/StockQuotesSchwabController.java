...
public class StockQuotesSchwabController {
    ...
    protected StockQuotesSchwabController() {
        try {
            ...
            this.marketDataService = new MarketDataService(apiProperties, tokenManager);
            
            log.info("Schwab services initialized manually (avoiding Spring proxy issues)");
    ...
    // Remove the loadYamlConfig method as it's no longer needed
...
//                    if (++i >= 3) break;
    ...
    public void getEquityDataFromList() {
        ...
        try {
            ...
            List<DailyPriceData> data = marketDataService
                    .getBulkHistoricalData(symbolsArray);
            schwabDailyPriceData.clear();
            schwabDailyPriceData.addAll(data);
            
            long successful = schwabDailyPriceData.stream().filter(DailyPriceData::isSuccess).count();
    ...
    private void doHistoricalSQL() {
        if (schwabDailyPriceData.isEmpty()) return;
        ...
        try (Connection con = CMDBController.getConnection();
            ...
            for (DailyPriceData data : schwabDailyPriceData) {
                ...
                try {
                    ...
                    pStmt.setDouble(4, data.getOpen() != null ? data.getOpen() : 0.0);
                    pStmt.setDouble(5, data.getHigh() != null ? data.getHigh() : 0.0);
                    pStmt.setDouble(6, data.getLow() != null ? data.getLow() : 0.0);
                    pStmt.setDouble(7, data.getClose() != null ? data.getClose() : 0.0);
                    pStmt.setLong(8, data.getVolume() != null ? data.getVolume() : 0L);
                    
                    pStmt.executeUpdate();
                    successCount++;
                    
                } catch (SQLException e) {
                    log.debug("Insert failed for {}: {}", data.getSymbol(), e.getMessage());
                    errorCount++;
                }
            }
            
            log.info("SQL insert completed: {} successful, {} errors", successCount, errorCount);
    ...
    public boolean isServiceReady() { return marketDataService.isReady(); }
    public String getTokenStatus() { return marketDataService.getTokenStatus(); }
    public int getSymbolCount() { return equitySymbolList.size(); }
    public int getDataCount() { return schwabDailyPriceData.size(); }
    public long getSuccessCount() { return schwabDailyPriceData.stream().filter(DailyPriceData::isSuccess).count(); }
